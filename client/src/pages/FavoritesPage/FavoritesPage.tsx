...
import { removeTicker } from '../../redux/tickerReducer';
...
export const FavoritesPage = () => {
  ...
  const handleTickerData = useCallback((data: Quote[]) => {
    ...
    const preparedData = data.filter(d => tickers.includes(d.ticker));

    preparedData.forEach((quote: Quote) => {
  ...
  if (!tickers.length) {
    return (
      <h2>There are no tickers in favorites yet</h2>
    );
  }
